...
class RestTiltedBehave(pypot.primitive.Primitive):
    def run(self):
        poppy = self.robot
        ...
        time.sleep(0.5)
        poppy.bust_x.goto_position(0,1,wait=True)
        time.sleep(0.5)
        ...
        poppy.l_shoulder_y.goto_position(-20, 1.5, wait=False)
        poppy.l_shoulder_x.goto_position(10, 1.5, wait=False)
        poppy.l_arm_z.goto_position(-20, 1.5, wait=False)
        poppy.l_elbow_y.goto_position(-15, 1.5, wait=Fal
