...
def findmouth(img):
  ...
  haarFace = cv2.cv.Load('haarcascade_frontalface_default.xml')
  haarMouth = cv2.cv.Load('haarcascade_mouth.xml')
  # running the classifiers
  storage = cv2.cv.CreateMemStorage()
  detectedFace = cv2.cv.HaarDetectObjects(cv2.cv.fromarray(img), haarFace, storage)
  detectedMouth = cv2.cv.HaarDetectObjects(cv2.cv.fromarray(img), haarMouth, storage)
