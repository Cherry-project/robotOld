...
def audio_extract(input_text='',args=None):
    ...
    #   or arg parse object
    if args is None:
        args = audio_args(language='fr-fr',output=open('output.mp3', 'w'))
    if type(args) is dict:
        args = audio_args(
                    language=args.get('language','fr-fr'),
    ...
    for idx, val in enumerate(combined_text):
        mp3url = "http://api.voicerss.org/?key=f4714f79ae944b5d9276593d2144dc0a&hl=fr-fr&src=%s" %(
            urllib.quote(val))
            #len(combined_text),
            #idx)
...
def text_to_speech_mp3_argparse():
    description = 'Voice RSS Downloader.'
...
if __name__ == "__main__":
    ...
    os.system('"C:/Program Files (x86)/VideoLAN/VLC/vlc" --play-and-exit -q ' + args.output.name)
